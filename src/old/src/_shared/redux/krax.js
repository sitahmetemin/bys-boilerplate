...
export function fetchUpdateParameters(parameters, actionSource, type, response, source) {
    if (actionSource[type] && !_.isEmpty(actionSource[type])) {
        actionSource = {
            ...
            [type]: {
                ...actionSource[type],
                messages: getInitialMessage(actionSource[type].messages, type)
            },
            value: response
        }
    } else {
...
export function getFetchBody(method, params) {
    ...
    if (params && method && (_.toUpper(method) === "POST" || _.toUpper(method) === "PUT")) {
        fetchBody = {
            method: _.toUpper(method),
            body: JSON.stringify(params),
            headers: header
        };
    } else if (params && method && (_.toUpper(method) === "DELETE" || _.toUpper(method) === "GET")) {
        fetchBody = {
            method: _.toUpper(method),
            headers: header
        };
...
export function getInitialMessage(message, type) {
    let messageArr = [];
